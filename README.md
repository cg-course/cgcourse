...
Теперь подробнее рассмотрим, как задаются атрибуты вершин. Предположим, у нас есть массив с заполенными данными вершин для какой-нибудь модели. В одном и том же буфере будут располагаться данные и положения, и цвета для каждой вершины, одна вершина сразу за другой. Рассмотрим пример задания атрибута положения. Я думаю, эта картинка все объяснит:
