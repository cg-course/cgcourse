...
Для каждого треугольника индекс-буфер содержит три числа: номера трех вершин в буффере вершин. Соответственно, теперь в буфере вершин (VBO) мы задаем уникальные вершины, а в индекс-буфере задаем треугольники.
...
Пример работы с индекс-буфером, а так же пример работы с матрицами вы найдете в проекте с названием **``cgbeginner-functions``**.
